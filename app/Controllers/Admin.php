...
public function view_feedback($id = false)
{
        ...
        public function delete_feedback($id = false)
    {
        ...
        {
            // Assuming you have a model named PertanyaanModel
            $email = $this->pengaduanModel->findAll();
            ...
            public function balas($id)
            {
                ...
                if (!$pengaduan) {
                    return redirect()->to('/email')->with('error', 'Pengaduan tidak ditemukan.');
                }
                ...
                if ($this->request->getMethod() == 'post') {
                    ...
                    // Coba kirim email dan periksa statusnya
                    ...
                    // Update status pengaduan menjadi "Ditanggapi"
                    $this->pengaduanModel->update($id, [
                        'status' => 'Ditanggapi',
                        'balasan' => $balasan,
                    ]);

                    return redirect()->to('/email')->with('success', 'Balasan berhasil dikirim dan email sudah terkirim.');
                }

                // Kirimkan data ke view balas
                $data = [
                ...
                return view('admin/balas', $data);
            }

        private function _sendEmail($emailTo, $balasan)
        {
            $email = \Config\Services::email();
            $email->setFrom('tahtaanugrah@gmail.com', 'Admin Pengaduan');
            $email->setTo($emailTo);
